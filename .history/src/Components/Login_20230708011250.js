...
function Login() {
            ...
            if(getUserData && getUserData.length){
                ...
                if(loginDataCheck.length === 0){
                  toast.error("invalid details",{position:"top-right"})
                    ...
                    localStorage.setItem("isfoodContainer")
                }
            }
        }
      }
  return (
    <div className="mainContainer">
        ...
        <input placeholder="Email" type="email" name="email" onChange={getData}/>
        <br />
        <input placeholder="Password" type="password" name="password" onChange={getData}/>
        <br />
        <Button type='submit' variant='contained' className='m-2'>Login</Button>
        <p>Don't have Accout <Link to="/" > Register </Link></p>
      </form>
      <ToastContainer/>
    </div>
  )
